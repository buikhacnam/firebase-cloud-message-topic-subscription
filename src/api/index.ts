...
API.interceptors.request.use((req: TODO) => {
	let accessToken = Cookies.get('fcm-access')
	let refreshToken = Cookies.get('fcm-refresh')
	let userName = Cookies.get('fcm-user')
	if (accessToken) {
...
export const saveUserDevices = (data: {userName: string, fcmToken: string, deviceType: string}) => {
	return API.post(`/api/v1/user-device/save`, data)
}

export const sendNotiToAllUsers = () => {
