...
import { saveUserDevices } from '../../../api'

export const loginUser = (user: {userName: string, password: string, remember: string}, navigate: NavigateFunction, fcmToken: string, deviceType:string) => {
	return async function (dispatch: Dispatch) {
		...
		try {
			...
			if (remember === 'on') {
				Cookies.set('fcm-access', access_token, { expires: 7 })
				Cookies.set('fcm-refresh', refresh_token, { expires: 7 })
				Cookies.set('fcm-user', userName, { expires: 7 })
			} else {
				Cookies.set('fcm-access', access_token)
				Cookies.set('fcm-refresh', refresh_token)
				Cookies.set('fcm-user', userName)
			}

			const resUserDevice = await saveUserDevices({userName, fcmToken, deviceType})
			console.log('resUserDevice', resUserDevice)
